...
public class Slice {
    ...
    public void normalizeCRAMRecords(final List<CRAMCompressionRecord> cramCompressionRecords,
        ...
        if (compressionHeader.isReferenceRequired()) {
            ...
            if (getAlignmentContext().getReferenceContext().isMappedSingleRef()) {
                ...
                if (!validateReferenceMD5(cramReferenceRegion)) {
    ...
    //VisibleForTesting
    boolean validateReferenceMD5(final CRAMReferenceRegion cramReferenceRegion) {
    ...
    private static boolean referenceMD5IsValid(
        ...
        // use an offset 0 here, which is correct given the assumption that we are ALWAYS using
        // a CRAMReferenceRegion that has bases that exactly match the span of the alignment context
